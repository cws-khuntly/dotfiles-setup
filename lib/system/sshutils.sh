...
(
    ...
    if [[ "${target_host}" == "$(hostname -s)" ]] || [[ "${target_host}" == "$(hostname -f)" ]] || \
        ...
        for keytype in "${SSH_HOST_KEYS[@]}"; do
            ...
            [[ -n "${does_key_exist}" ]] && builtin unset -v does_key_exist;
            ...
            if [[ -z "${ret_code}" ]] || [[ -z "${returned_info}" ]] || (( ret_code != 0 )); then
                ...
                [[ -n "${remote_ssh_version}" ]] && builtin unset -v remote_ssh_version;
                [[ -n "${remote_ssh_key}" ]] && builtin unset -v remote_ssh_key;
                [[ -n "${ret_code}" ]] && builtin unset -v ret_code;
                ...
                [[ -n "${keytype}" ]] && builtin unset -v keytype;
                [[ -n "${does_key_exist}" ]] && builtin unset -v does_key_exist;
                [[ -n "${remote_ssh_key}" ]] && builtin unset -v remote_ssh_key;
        fi
    ...
    [[ -n "${known_hosts_file}" ]] && builtin unset -v known_hosts_file;
...
(
    ...
    if [[ -z "${return_code}" ]] || (( return_code == 0 )); then
        for available_ssh_key_type in "${SSH_KEY_TYPES[@]}"; do
            ...
            [[ -n "${ssh_key_type}" ]] && builtin unset -v ssh_key_type;
            [[ -n "${ssh_key_size}" ]] && builtin unset -v ssh_key_size;
            [[ -n "${ssh_key_filename}" ]] && builtin unset -v ssh_key_filename;
            [[ -n "${ret_code}" ]] && builtin unset -v ret_code;
        done
    fi
    ...
    [[ -n "${ssh_key_size}" ]] && builtin unset -v ssh_key_size;
    [[ -n "${ssh_key_filename}" ]] && builtin unset -v ssh_key_filename;
    [[ -n "${available_ssh_key_type}" ]] && builtin unset -v available_ssh_key_type;
    [[ -n "${ret_code}" ]] && builtin unset -v ret_code;
...
(
    ...
    if [[ -n "${SSH_KEY_LIST[*]}" ]] && (( ${#SSH_KEY_LIST[*]} != 0 )); then
        ...
        if [[ -z "${ret_code}" ]] || [[ -z "${returned_info}" ]] || (( ret_code != 0)); then
            ...
            for keyfile in "${SSH_KEY_LIST[@]}"; do
                ...
                [[ -n "${ret_code}" ]] && builtin unset -v ret_code;
                [[ -n "${keyfile}" ]] && builtin unset -v keyfile;
            done
        fi
    else
        return_code=1;
    ...
    [[ -n "${ret_code}" ]] && builtin unset -v ret_code;
    [[ -n "${continue_exec}" ]] && builtin unset -v continue_exec;
    [[ -n "${target_host}" ]] && builtin unset -v target_host;
    [[ -n "${target_port}" ]] && builtin unset -v target_port;
...
(
    ...
    [[ -n "${error_count}" ]] && builtin unset -v error_count;
    [[ -n "${ret_code}" ]] && builtin unset -v ret_code;
    [[ -n "${sshconfig}" ]] && builtin unset -v sshconfig;
    [[ -n "${sshkey}" ]] && builtin unset -v sshkey;
    [[ -n "${target_host}" ]] && builtin unset -v target_host;
    [[ -n "${target_port}" ]] && builtin unset -v target_port;
    [[ -n "${target_user}" ]] && builtin unset -v target_user;
    [[ -n "${target_transport}" ]] && builtin unset -v -v target_transport;
    [[ -n "${run_cmd}" ]] && builtin unset -v run_cmd;
    [[ -n "${cmd_output}" ]] && builtin unset -v cmd_output;
...
(
    ...
    [[ -n "${target_user}" ]] && builtin unset -v target_user;
    [[ -n "${target_transport}" ]] && builtin unset -v target_transport;
    [[ -n "${sftpfile}" ]] && builtin unset -v sftpfile;
    [[ -n "${cmd_output}" ]] && builtin unset -v cmd_output;
